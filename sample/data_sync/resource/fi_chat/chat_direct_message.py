...
def generate_period():
  body = data_sync.libs.utils.get_period_from_s3('chat_direct_message')
...
def save_period(period_from, period_to):
  ...
  data_sync.libs.utils.put_period_to_s3('chat_direct_message', body)
...
def extract(period_from, period_to):
  ...
  query           = data_sync.libs.constant.QUERY_CHAT_DIRECT_MESSAGES.format(period_from=period_from, period_to=period_to)
...
def transform(dynamic_frame):
  ...
  resource_type         = 'chat_direct_message'
  ...
  related_resource_type = 'chat_direct_message'
  ...
  dataframe = dataframe.withColumn('resource_name', resource_name(lit('ChatDirectRoom'), dataframe['chat_direct_room_id'], lit('ChatDirectMessage'), dataframe['uuid']))
  dataframe = dataframe.withColumn('related_resource_type', lit(related_resource_type))
  dataframe = dataframe.withColumn('related_resource_name', related_resource_name(lit('ChatDirectRoom'), dataframe['chat_direct_room_id']))
  ...
  dataframe = dataframe.withColumn('user_id', dataframe['sender_id']) # requl_mobile の user_id, ≠ finc_user_id
  return DynamicFrame.fromDF(dataframe, glueContext, 'chat_direct_messages')
...
def load(dynamic_frame):
  data_sync.libs.utils.put_payload_to_s3('chat_direct_message', dynamic_frame)

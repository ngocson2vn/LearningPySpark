...
def generate_period():
  body = data_sync.libs.utils.get_period_from_s3('snap_comment')
...
def save_period(period_from, period_to):
  ...
  data_sync.libs.utils.put_period_to_s3('snap_comment', body)
...
def extract(period_from, period_to):
  ...
  query           = data_sync.libs.constant.QUERY_SNAP_COMMENTS.format(period_from=period_from, period_to=period_to)
...
def transform(dynamic_frame):
  ...
  resource_type         = 'snap_comment'
  resource_name         = udf(lambda snap_id, id: '/snaps/%s/comments/%s' % (snap_id, id))
  related_resource_type = 'snap'
  related_resource_name = udf(lambda snap_id: '/snaps/%s' % snap_id)
  ...
  dataframe = dataframe.withColumn('resource_name', resource_name(dataframe['user_activity_id'], dataframe['id']))
  dataframe = dataframe.withColumn('related_resource_type', lit(related_resource_type))
  dataframe = dataframe.withColumn('related_resource_name', related_resource_name(dataframe['user_activity_id']))
  ...
  return DynamicFrame.fromDF(dataframe, glueContext, "comments")

def select_fields(dynamic_frame):
  return data_sync.libs.utils.select_fields(dynamic_frame)

def load(dynamic_frame):
  data_sync.libs.utils.put_payload_to_s3('snap_comment', dynamic_frame)
